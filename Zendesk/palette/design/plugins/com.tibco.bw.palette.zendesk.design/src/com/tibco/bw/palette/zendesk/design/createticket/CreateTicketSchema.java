...
public class CreateTicketSchema extends BWExtensionActivitySchema 

{
	
	private static String ATTRIBUTE_NAME = "source"; //$NON-NLS-1$
	private static String ATTRIBUTE_VALUE = "bw.zendesk.createTicket"; //$NON-NLS-1$
	
	private static CreateTicketSchema INSTANCE = new CreateTicketSchema();
    /**
	 * <!-- begin-custom-doc -->
	 * 
	 * <!-- end-custom-doc -->
	 ...
	 * File location.
	 */
	private static final String SCHEMA_FILE_PATH = "/schema/CreateTicket.xsd";
    /**
	 * <!-- begin-custom-doc -->
	 * 
	 * <!-- end-custom-doc -->
			...
			if (particle != null && particle.getTerm() instanceof XSDModelGroup) {
				if (matchExtension) {
					if (hasExtensionAttribute(xsdTypeDefinition, ATTRIBUTE_NAME, ATTRIBUTE_VALUE)) {
						...
						return BWDesign.createErrorStatus(Messages.CreateTicketSchema_inputHeader_sourceError_label);
					}
				} else {
					isValid = true;
				}
	...
	public XSDSchema createInputHeaderSchemaWithType() {
		String prefix = Messages.CreateTicketSchema_inputHeader_tns_prefix;
		String namespace = Messages.CreateTicketSchema_inputHeader_tns + "/" + UUID.randomUUID().toString();
