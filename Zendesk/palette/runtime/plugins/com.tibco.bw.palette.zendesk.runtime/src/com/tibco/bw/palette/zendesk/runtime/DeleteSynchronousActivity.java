	...
	private boolean validateIds(String type, long[] idList, boolean exists) {
		...
		for(long id : idList) {
			...
			case "user" :
				...
				if(user != null && user.getEmail() != null && exists)  // id exists before delete, so return true
					validate = true;
				else if((user == null || user.getEmail() == null) && exists) // id does not exist before delete, implies invalid input
					validate = false;
				else if(user != null && user.getEmail() != null && !exists) // id exists after delete, implies delete failed
					validate = false;
				else if((user == null || user.getEmail() == null) && !exists) // id does not exist after delete, implies delete succeeded
					validate = true;
				break;
